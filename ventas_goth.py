...
with tab_bazar:
    ...
    for i, (id_b, info_b) in enumerate(bloques_activos.items()):
        with cols[i % 3]:
            ...
            html_imagenes = ""
            if 'imagenes' in info_b and info_b['imagenes']:
                for img_file in info_b['imagenes']:
                    b64_img = base64.b64encode(img_file.getvalue()).decode()
                    html_imagenes += f'<img src="data:image/png;base64,{b64_img}" style="width:60px; height:60px; object-fit:cover; margin:2px; border-radius:5px; border: 1px solid #5d0f75;">'
            ...
            st.markdown(f"""
                <div class="shein-card">
                    ...
                    <div style="margin-top:10px;">{html_imagenes}</div>
                </div>
            """, unsafe_allow_html=True)
