...
# --- 4. ESTILOS CSS (ESTÉTICA GOTH: OSCURO, NEGRO Y MORADO) ---
st.markdown("""
    <style>
    /* Fondo general de la App - Estilo Goth */
    .stApp {
        background: radial-gradient(circle at center, #2a0845 0%, #050505 70%) !important;
    }
    
    /* ========================================================
       ✨ CONTENEDOR DE PERFIL ESTILO BAZAR GOTH ✨
       ======================================================== */
    .fb-header-container {
        position: relative;
        width: 100%;
        ...
        border-radius: 24px;
        padding: 15px;
        box-shadow: 0 10px 32px 0 rgba(93, 15, 117, 0.3);
        margin-bottom: 30px;
        overflow: hidden;
    }
    ...
    /* Foto de Perfil Redonda con borde grueso */
    .fb-profile-avatar {
        width: 140px;
        height: 140px;
        border-radius: 50%;
        border: 5px solid #2a0845;
    ...
    /* Textos alineados abajo */
    .fb-profile-info {
        margin-left: 25px;
        margin-top: 50px;
    }
    
    /* ✨ TÍTULO CON DEGRADADO GOTH ✨ */
    .gradient-title {
        font-size: 34px !important;
        font-weight: 900 !important;
        background: linear-gradient(45deg, #b39ddb, #e040fb) !important;
    ...
    /* ========================================================
       TARJETAS Y CONTENIDOS (ADAPTADOS A MODO OSCURO)
       ======================================================== */
    .stForm, .preview-container, .public-block, .admin-box {
        background-color: rgba(20, 20, 25, 0.85) !important;
        padding: 20px;
        border-radius: 15px;
        border: 1px solid #5d0f75;
        box-shadow: 0px 4px 20px rgba(0, 0, 0, 0.4);
        margin-bottom: 25px;
    }
    
    .shein-card {
        background-color: rgba(30, 10, 40, 0.4) !important;
        border: 2px solid #5d0f75 !important;
        border-radius: 12px;
        padding: 15px;
        margin-bottom: 20px;
    ...
    /* Textos generales dentro de las tarjetas */
    ...
    div[data-testid="stFileUploader"] section * { color: #d1d1d1 !important; }
    div[data-testid="stFileUploader"] button { background-color: #5d0f75 !important; color: #FFFFFF !important; }
    ...
    div[data-testid="stFormSubmitButton"] button, .stSubmitButton button, div.stSubmitButton > button {
        ...
        width: 100% !important;
        box-shadow: 0px 6px 15px rgba(123, 31, 162, 0.4) !important;
    }
    div[data-testid="stFormSubmitButton"] button *, .stSubmitButton button *, div.stSubmitButton > button * {
        color: #FFFFFF !important;
    ...
    .articulos-box-shein {
        background-color: #10051a !important;
        color: #d1d1d1 !important;
        padding: 10px;
        border-radius: 6px;
    ...
    .badge-activo-shein {
        background-color: #2e7d32; color: #ffffff; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: bold; display: inline-block;
    }
    ...
    /* Aviso de seguridad tenue modo oscuro */
    .alerta-seguridad-principal {
        background-color: rgba(42, 8, 69, 0.4) !important;
        color: #d1d1d1 !important;
        padding: 18px;
        border-radius: 14px;
...
# --- 5. RENDERIZADO DEL ENCABECADO ---
fondo_portada_fallback = img_portada_base64 if img_portada_base64 else "linear-gradient(90deg, #2a0845, #5d0f75)"
fondo_perfil_fallback = img_perfil_base64 if img_perfil_base64 else "linear-gradient(135deg, #311b92, #4a148c)"
...
st.markdown(f"""
    <div class="fb-header-container">
        ...
        <div class="fb-profile-row">
            <div class="fb-profile-avatar" style="background: {fondo_perfil_fallback if not img_perfil_base64 else '#2a0845'}; display: flex; align-items: center; justify-content: center;">
...
tab_bazar, tab_anunciarse, tab_admin = st.tabs(["🛍️ Ver el Bazar", "💜 Registrarse como Vendedora", "🔐"])
    ...
    if not bloques_activos:
        ...
        for i in range(0, len(lista_rotada), columnas_por_fila):
            ...
            for idx_col, (id_b, info_b) in enumerate(fila_bloques):
                with cols[idx_col]:
                    texto_mensaje = "Hola, vengo del Bazar Nocturnal Goth, me interesó alguno de tus artículos. ✨🛍️"
                        ...
                        <div class="shein-card">
                            <div>
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                    ...
                                    <span style="font-size: 11px; color: #aaaaaa;">📅 {info_b['fecha']}</span>
                                </div>
                                <h4 style="margin: 0 0 5px 0; color:#ab47bc; font-size: 18px;">🛍️ Bazar de {info_b['vendedor']}</h4>
                                <p style="margin: 2px 0; color:#d1d1d1; font-size: 12px;">📂 <b>Categoría:</b> {info_b['categoria']}</p>
                                <p style="margin: 2px 0; color:#d1d1d1; font-size: 12px; margin-bottom: 10px;">📍 <b>Punto:</b> {info_b['zona']}</p>
                                <div class="articulos-box-shein">{info_b['articulos']}</div>
                            </div>
                    """, unsafe_allow_html=True)
                    
                    if info_b.get('imagenes'):
                        st.markdown("<span style='font-size:12px; color:#d1d1d1;'>📸 Fotos:</span>", unsafe_allow_html=True)
                    ...
                    st.markdown(f"""
                            <div style="margin-top: 15px;">
                                <a href="{url_wa_vendedor}" target="_blank" style="text-decoration: none;">
                                    <button style="background-color:#ab47bc; color:white; border:none; padding:10px 15px; font-weight:bold; border-radius:8px; cursor:pointer; width:100%; font-size:13px;">
                                        💬 Contactar por WhatsApp
                                    </button>
                                </a>
                                <div style="text-align: center; color: #b39ddb; font-weight: bold; font-size: 12px; margin-top: 8px; margin-bottom: 8px;">
                                    ✨ ¡Gracias por tu preferencia! ✨
                                </div>
                                <hr style="border: 0; height: 5px; background-color: #ab47bc; margin: 0; border-radius: 5px;">
                            </div>
                        </div>
                        <br>
                    """, unsafe_allow_html=True)
...
with tab_anunciarse:
    ...
    with st.form("form_anuncio", clear_on_submit=True):
        ...
        st.markdown("""
            <div style="background-color: #1a0933; padding: 20px; border-radius: 10px; border: 2px solid #ab47bc;">
                <p style="color: #b39ddb !important; font-size: 17px !important; margin: 0 0 5px 0; font-weight: 900;">🏛️ BANCO: NU MÉXICO</p>
                <p style="color: #ffffff !important; font-size: 17px !important; margin: 0 0 5px 0; font-family: monospace; font-weight: bold;">🔑 CLABE: 0123 4567 8901 2345 67</p>
                <p style="color: #ffffff !important; font-size: 17px !important; margin: 0; font-weight: bold;">👤 TITULAR: CAPITANA ALBATROS</p>
            </div>
        """, unsafe_allow_html=True)
        st.write("")
    ...
    if st.session_state.pre_registro is not None:
        ...
        if id_b in st.session_state.bloques_db and st.session_state.bloques_db[id_b]['estado'] == "🟢 ACTIVO":
            ...
            msg = (
                f"Hola, vengo de la página del Bazar Nocturnal Goth.\n\n"
    ...
    if clave_ingresada == CONTRASENA_ADMIN:
        ...
        if not st.session_state.bloques_db:
            ...
            for b_id in list(st.session_state.bloques_db.keys()):
                ...
                st.markdown(f"""
                    <div class="admin-box">
                        <span style="color:#ab47bc;"><b>ID Solicitud:</b> {b_id}</span><br>
